
part of 'screen_time_session.dart';

class ScreenTimeSessionAdapter extends TypeAdapter<ScreenTimeSession> {
  @override
  final int typeId = 14;

  @override
  ScreenTimeSession read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return ScreenTimeSession(
      id: fields[0] as String,
      appName: fields[1] as String,
      category: fields[2] as AppCategory,
      startTime: fields[3] as DateTime,
      endTime: fields[4] as DateTime?,
      durationMinutes: fields[5] as int,
      limitMinutes: fields[6] as int,
      limitExceeded: fields[7] as bool,
    );
  }

  @override
  void write(BinaryWriter writer, ScreenTimeSession obj) {
    writer
      ..writeByte(8)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.appName)
      ..writeByte(2)
      ..write(obj.category)
      ..writeByte(3)
      ..write(obj.startTime)
      ..writeByte(4)
      ..write(obj.endTime)
      ..writeByte(5)
      ..write(obj.durationMinutes)
      ..writeByte(6)
      ..write(obj.limitMinutes)
      ..writeByte(7)
      ..write(obj.limitExceeded);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ScreenTimeSessionAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class AppCategoryAdapter extends TypeAdapter<AppCategory> {
  @override
  final int typeId = 13;

  @override
  AppCategory read(BinaryReader reader) {
    switch (reader.readByte()) {
      case 0:
        return AppCategory.distraction;
      case 1:
        return AppCategory.productive;
      case 2:
        return AppCategory.neutral;
      default:
        return AppCategory.distraction;
    }
  }

  @override
  void write(BinaryWriter writer, AppCategory obj) {
    switch (obj) {
      case AppCategory.distraction:
        writer.writeByte(0);
        break;
      case AppCategory.productive:
        writer.writeByte(1);
        break;
      case AppCategory.neutral:
        writer.writeByte(2);
        break;
    }
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is AppCategoryAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
